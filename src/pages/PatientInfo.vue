    ...
    <el-container>
      ...
      <el-main>
        <el-table class="patient-info-table" :data="showList">
          <el-table-column prop="patientNum" label="病例号" width="120" />
          <el-table-column prop="patientName" label="姓名" width="80" />
          <el-table-column prop="gender" label="性别" width="60" />
          <el-table-column prop="age" label="年龄" width="60" />
          <el-table-column prop="patientTel" label="电话" width="150" />
          <el-table-column prop="roomNum" label="房间号" width="70" />
          <el-table-column prop="result" label="诊断结果" width="300" />
          <el-table-column prop="doctorName" label="医生姓名" width="100" />
          <el-table-column prop="department" label="科室" width="80" />
          <el-table-column fixed="right" label="操作">
            <template #default>
              <el-button link type="primary" size="small" @click="EditPatientInfo()">编辑</el-button>
              <el-button link type="primary" size="small" @click="getPatientDetail()">详细信息</el-button>
            </template>
          </el-table-column>
        </el-table>
      </el-main>  
...
var showList = computed(()=>{
  return dataList.filter(obj=>{
    return String(obj.patientName).indexOf(searchPatientName.value) > -1 && String(obj.doctorName).indexOf(searchDoctorName.value) > -1
  })
})
